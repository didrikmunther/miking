...
let fileExtMap =
  ...
  [
    ...
    ("readBytes", [
      { expr = "(fun rc len -> try let buf = Bytes.create len in let actual_len = input rc buf 0 len in let reached_eof = actual_len < len in let had_error = false in (Bytes.sub_string buf 0 actual_len, reached_eof, had_error) with | Sys_error err -> (\"\", false, true))",
        ty = tyarrows_ [otyvarext_ "in_channel" [], tyint_, otytuple_ [otystring_, tybool_, tybool_]],
        libraries = [],
        cLibraries = []
      }
    ]),
